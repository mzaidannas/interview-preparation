...
class Metrices
  ...
  def call
    incidents.each do |incident|
      ...
      [total_for_each_train[incident.train_number], max_for_train].max
      unknown_incidents << "day: #{incident.incident_date.split('-')[2]} hour: #{incident.incident_time.split(':')[0]}" if incident.defect_id.to_i == 42
